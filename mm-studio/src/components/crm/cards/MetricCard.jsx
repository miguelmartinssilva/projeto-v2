...
export default function MetricCard({ label, value, icon: Icon, color, change, delay = 0 }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay, duration: 0.3 }}
      ...
      <div className="absolute top-0 left-0 right-0 h-px opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: `linear-gradient(90deg, transparent, ${color}, transparent)` }} />
      ...
      <p className="text-lg font-bold text-text leading-none mb-1">{value}</p>
