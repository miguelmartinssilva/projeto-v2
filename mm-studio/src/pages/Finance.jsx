...
const CATEGORIAS = ["Ferramentas","Software","Marketing","Equipamento","Transporte","Alimentacao","Outros"];
const CATEGORY_COLORS = { Ferramentas: "#ff4d6d", Software: "#7c3aed", Marketing: "#448aff", Equipamento: "#ffb800", Transporte: "#ff6b35", Alimentacao: "#69f0ae", Outros: "#888" };
const CATEGORY_LABELS = { Ferramentas: "Ferramentas", Software: "Software", Marketing: "Marketing", Equipamento: "Equipamento", Transporte: "Transporte", Alimentacao: "Alimentacao", Outros: "Outros" };
    ...
    <div className="bg-bg-elevated border border-border-card rounded-xl px-4 py-3 text-sm shadow-2xl">
      ...
      {payload.map((p) => (
        <div key={p.name} className="flex items-center gap-2 text-xs py-0.5">
          ...
          <span className="text-text-secondary">{p.name === "receita" ? "Receita" : p.name === "despesa" ? "Despesa" : p.name}:</span>
...
function calcFixoRevenue(fixos) {
  const byMonth = {};
  fixos.forEach(f => {
    (f.historicoFinanceiro || []).forEach(h => {
      if (h.status === "pago") {
        const [ano, mes] = h.mes.split("-");
        const key = `${ano}-${parseInt(mes)}`;
        byMonth[key] = (byMonth[key] || 0) + (h.valor || 0);
      }
    });
  });
  return byMonth;
...
export default function Finance() {
  ...
  const { topCards, chartData, pieData, pendings, fixoTotal } = useMemo(() => {
    const now = new Date();
    const curMonth = now.getMonth();
    const curYear = now.getFullYear();
    const txns = getTransactions();
    const despesasRaw = getDespesas();
    const fixos = getFixos();

    const saidas = [
      ...txns.filter(t => t.tipo === "saida"),
      ...despesasRaw.filter(d => !txns.some(t => t.tipo === "saida" && t.id === d.id)).map(d => ({ ...d, tipo: "saida" })),
    ];
    const allTxns = [...txns.filter(t => t.tipo !== "saida"), ...saidas];

    const fixoByMonth = calcFixoRevenue(fixos);
    const totalFixoRevenue = Object.values(fixoByMonth).reduce((s, v) => s + v, 0);

    const totalEntradas = txns.filter(t => t.tipo === "entrada").reduce((s, t) => s + (t.valor || 0), 0) + totalFixoRevenue;
    ...
    }).reduce((s, t) => s + (t.valor || 0), 0) + (fixoByMonth[`${curMonth === 0 ? curYear - 1 : curYear}-${curMonth === 0 ? 11 : curMonth - 1}`] || 0);
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const fixoVal = fixoByMonth[`${y}-${m}`] || 0;
      chart.push({ mes: MESES[m], receita: receita + fixoVal, despesa });
      fixoChart.push({ mes: MESES[m], fixos: fixoVal });
    }
    ...
    if (totalFixoRevenue > 0) catMap["clientes-fixos"] = (catMap["clientes-fixos"] || 0) + totalFixoRevenue;
    const totalCat = Object.values(catMap).reduce((s, v) => s + v, 0) || 1;
    const pie = Object.entries(catMap).map(([name, value], i) => ({
      name: name === "clientes-fixos" ? "Clientes Fixos" : name.replace(/-/g, " ").replace(/\b\w/g, l => l.toUpperCase()),
      value: Math.round(value / totalCat * 100),
      color: name === "clientes-fixos" ? "#ffb800" : PIE_COLORS[i % PIE_COLORS.length],
    }));
    ...
    return { topCards: cards, chartData: chart, pieData: pie, pendings: pends, fixoTotal: totalFixoRevenue };
  }, []);
  ...
  const totalMes = useMemo(() => {
    const d = new Date();
    return despesas.filter(dd => { const dt = new Date(dd.data + "T12:00:00"); return dt.getMonth() === d.getMonth() && dt.getFullYear() === d.getFullYear(); }).reduce((s, dd) => s + (dd.valor || 0), 0);
  }, [despesas]);
  const totalAno = useMemo(() => {
    const y = new Date().getFullYear();
    return despesas.filter(dd => new Date(dd.data + "T12:00:00").getFullYear() === y).reduce((s, dd) => s + (dd.valor || 0), 0);
  }, [despesas]);
  const maiorValor = useMemo(() => Math.max(...despesas.map(d => d.valor || 0), 0), [despesas]);
  const mediaMensal = useMemo(() => {
    const months = new Set(); despesas.forEach(d => { const dt = new Date(d.data + "T12:00:00"); months.add(`${dt.getFullYear()}-${dt.getMonth()}`); });
    const count = months.size || 1;
    return despesas.reduce((s, d) => s + (d.valor || 0), 0) / count;
  }, [despesas]);
  const pieDespesas = useMemo(() => {
    const map = {}; despesas.forEach(d => { const cat = d.categoria || "Outros"; map[cat] = (map[cat] || 0) + (d.valor || 0); });
    const total = Object.values(map).reduce((s, v) => s + v, 0) || 1;
    return Object.entries(map).map(([name, value]) => ({ name: CATEGORY_LABELS[name] || name, value: Math.round(value / total * 100), color: CATEGORY_COLORS[name] || "#888" }));
  }, [despesas]);

  const openNewExpense = () => { setEditExpense(null); setShowExpenseModal(true); };
  const openEditExpense = (d) => { setEditExpense(d); setShowExpenseModal(true); };
  ...
  const confirmDel = () => {
    if (confirmDelete) {
      ...
      setTimeout(() => {
        ...
        const key = "financeiro_miguel";
        try { localStorage.setItem(key, JSON.stringify(txns)); } catch { /* ignore */ }
    ...
    <div className="flex-1 min-h-screen page-enter">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {topCards.map((c, i) => (
            <motion.div key={c.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 * i }} className={`bg-bg-card rounded-xl p-5 ${c.borderClass} glow-primary`}>
        ...
        {fixoTotal > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-bg-card rounded-xl p-5 mb-6 border-top-amber glow-primary">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: "#ffb80018" }}>
                <Star size={18} style={{ color: "#ffb800" }} />
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.12em] text-text-muted font-medium">Receita Recorrente (Clientes Fixos)</p>
                <p className="font-display text-xl font-bold" style={{ color: "#ffb800" }}>
                  R$ {fixoTotal.toLocaleString("pt-BR", { minimumFractionDigits: 2 })} <span className="text-xs font-normal text-text-muted">incluido na Receita Total</span>
                </p>
              </div>
            </div>
          </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-bg-card rounded-xl p-6 card-border glow-warning mb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2 bg-bg-card rounded-xl p-6 card-border glow-primary">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <select value={filterMes} onChange={(e) => setFilterMes(+e.target.value)} className="bg-bg-elevated border border-border-card rounded-lg px-2.5 py-1.5 text-xs text-text outline-none">
                  {MESES_FULL.map((m, i) => <option key={i} value={i}>{m}</option>)}
                </select>
                <select value={filterAno} onChange={(e) => setFilterAno(+e.target.value)} className="bg-bg-elevated border border-border-card rounded-lg px-2.5 py-1.5 text-xs text-text outline-none">
                  {[now.getFullYear(), now.getFullYear() - 1, now.getFullYear() - 2].map(a => (<option key={a} value={a}>{a}</option>))}
                </select>
                <select value={filterCat} onChange={(e) => setFilterCat(e.target.value)} className="bg-bg-elevated border border-border-card rounded-lg px-2.5 py-1.5 text-xs text-text outline-none">
                  <option value="Todas">Todas Categorias</option>
                  {CATEGORIAS.map(c => <option key={c} value={c}>{CATEGORY_LABELS[c]}</option>)}
                </select>
                <button onClick={() => { setSortDir(sortDir === "asc" ? "desc" : "asc"); }} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-border-card bg-bg-elevated text-xs text-text-muted hover:text-text transition-colors">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  <AnimatePresence>
                  {filtered.map((d) => (
                    <motion.tr key={d.id} layout exit={{ opacity: 0, x: 60, scale: 0.95, transition: { duration: 0.2 } }} className="border-b border-border-card/40 transition-colors hover:bg-white/[0.02] group">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-bg-card rounded-xl p-6 card-border glow-primary">
            ...
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={pieDespesas} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={3} dataKey="value">
...
function ExpenseModal({ item, onClose }) {
  ...
  const handleSave = () => {
    ...
    const trans = { id: obj.id, tipo: "saida", cliente: obj.descricao, categoria: obj.categoria, valor: obj.valor, data: obj.data, status: "pago" };
