...
import useAutomacoesStore, { STATUS_AUTO, INTEGRACOES, RESPONSAVEIS } from "../../../store/automacoesStore";

export default function FiltersDropdown() {
  const { statusFilter, integracaoFilter, responsavelFilter, setStatusFilter, setIntegracaoFilter, setResponsavelFilter } = useAutomacoesStore();
  const [open, setOpen] = useState(false);
  const hasFilters = statusFilter !== "todos" || integracaoFilter !== "todos" || responsavelFilter !== "todos";

  const clear = () => { setStatusFilter("todos"); setIntegracaoFilter("todos"); setResponsavelFilter("todos"); };
            ...
            <motion.div initial={{ opacity: 0, y: -4, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.95 }}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-1 flex-wrap mt-1">
                    <button onClick={() => setIntegracaoFilter("todos")} className={`px-2 py-1 rounded-md text-[10px] font-medium transition-all ${integracaoFilter === "todos" ? "bg-primary/15 text-primary" : "bg-white/5 text-text-muted hover:text-text"}`}>Todos</button>
                    {INTEGRACOES.map(i => (
                      <button key={i.id} onClick={() => setIntegracaoFilter(i.id)} className={`px-2 py-1 rounded-md text-[10px] font-medium transition-all ${integracaoFilter === i.id ? "" : "bg-white/5 text-text-muted hover:text-text"}`}
                        style={integracaoFilter === i.id ? { background: i.color + "18", color: i.color } : {}}>{i.nome}</button>
                    ))}
                  </div>
                </div>
                <div>
